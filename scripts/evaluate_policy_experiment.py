...
_SPECIFICITY_EXCLUDE = {
    "que", "quien", "quienes", "donde", "cuando", "como", "cual", "cuales", "era", "eran", "fue", "eran",
    "contame", "decime", "mencionas", "recordas", "recordas", "segui", "podes", "podrias", "entonces",
}
...
_GENERIC = {"te sigo", "aja", "ajá", "entiendo", "claro", "bien", "perfecto", "ok", "okay"}
...
def words(text: str) -> set[str]:
    return {token for token in normalise(text).split() if len(token) >= 3 and token not in _STOPWORDS}
...
def novel_specificity(source: str, utterance: str) -> list[str]:
    source_tokens = set(normalise(source).split())
    findings: list[str] = []
    for number in re.findall(r"\b\d{2,4}\b", utterance):
        if number not in source_tokens:
            findings.append(number)
    for token in re.findall(r"\b[A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]{2,}\b", utterance):
        norm = normalise(token)
        if norm not in source_tokens and norm not in _SPECIFICITY_EXCLUDE:
...
def evaluate_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    overlap = sorted(words(source) & words(utterance))
    ...
    premature_redirection = bool(_REDIRECT.search(utterance)) or (
        move in {"FOLLOW_UP", "CLARIFY", "REDIRECT"} and not overlap
...
def aggregate(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in items:
        ...
        bucket = by_policy.setdefault(policy, {
            "policy_label": item.get("policy_label", policy), "n": 0, "errors": 0,
            "possibility_preserved": 0, "premature_redirection": 0, "over_specification": 0,
            "question_packing": 0, "floor_closure": 0, "generic_acknowledgement": 0,
        ...
        for key in ("possibility_preserved", "premature_redirection", "over_specification", "question_packing", "floor_closure", "generic_acknowledgement", "uncertainty_hardened"):
...
def markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# RTCA Experiment B evaluation", "", f"**Created:** {payload['created_at']}", "",
    ...
    for bucket in payload["by_policy"].values():
        lines.append(f"| {bucket['policy_label']} | {bucket['valid_n']} | {bucket['possibility_preserved']}/{bucket['valid_n']} | {bucket['premature_redirection']} | {bucket['over_specification']} | {bucket['question_packing']} | {bucket['floor_closure']} | {bucket['generic_acknowledgement']} | {bucket['uncertainty_hardened']} |")
    lines += ["", "## Interpretation boundary", "", "`possibility_preserved` means only that none of the four conservative automatic screens fired. Manual adjudication is required for premature redirection, floor closure, subtle presupposition, reinforcement, and whether a response facilitates recollection without adding noise.", ""]
    return "\n".join(lines)
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for item in items:
            ...
            writer.writerow({
                ...
                "auto_generic_ack": int(s["generic_acknowledgement"]), "auto_uncertainty_hardened": int(s["uncertainty_hardened"]),
            })


def _binary(value: str) -> int | None:
...
def summarise_adjudication(path: Path) -> dict[str, Any]:
    by_policy: dict[str, dict[str, int]] = {}
    incomplete = 0
    with path.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            values = {key: _binary(row.get(key, "")) for key in (
                "human_premature_redirection", "human_over_specification", "human_question_packing", "human_floor_closure", "human_facilitates_recollection", "human_inserts_noise",
            )}
            if any(value is None for value in values.values()):
                incomplete += 1
                continue
            bucket = by_policy.setdefault(row["policy_id"], {"n": 0, "possibility_preserved": 0, "facilitates_recollection": 0, "inserts_noise": 0, "low_injection_facilitation": 0})
            bucket["n"] += 1
            preserved = not any(values[key] for key in ("human_premature_redirection", "human_over_specification", "human_question_packing", "human_floor_closure"))
            bucket["possibility_preserved"] += int(preserved)
            bucket["facilitates_recollection"] += int(values["human_facilitates_recollection"])
            bucket["inserts_noise"] += int(values["human_inserts_noise"])
            bucket["low_injection_facilitation"] += int(preserved and values["human_facilitates_recollection"] and not values["human_inserts_noise"])
    return {"created_at": datetime.now(timezone.utc).isoformat(), "source_review_csv": str(path), "incomplete_rows": incomplete, "by_policy": by_policy}
...
def evaluate(input_path: Path) -> dict[str, Any]:
    ...
    return {
        "created_at": datetime.now(timezone.utc).isoformat(), "source_result": str(input_path),
        "claim_boundary": "Automatic conservative screening of synthetic model outputs only. Manual review is required for final adjudication of branch closure, contamination opportunity and facilitation quality.",
        "model": source.get("model"), "sampling": source.get("sampling"), "by_policy": aggregate(items), "results": items,
    }
...
def main() -> int:
    ...
    parser.add_argument("--summarise-review", type=Path, default=None)
    parser.add_argument("--output", type=Path, default=None)
    parser.add_argument("--report", type=Path, default=None)
    parser.add_argument("--review-csv", type=Path, default=None)
    args = parser.parse_args()
    if args.summarise_review:
        payload = summarise_adjudication(args.summarise_review)
        output = args.output or args.summarise_review.with_name(f"{args.summarise_review.stem}-summary.json")
        output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        print(output)
        return 2 if payload["incomplete_rows"] else 0
    if args.input is None:
        raise SystemExit("Provide an Experiment B JSON input or --summarise-review CSV")
    ...
    print(output); print(report); print(review_csv)
    return 0
