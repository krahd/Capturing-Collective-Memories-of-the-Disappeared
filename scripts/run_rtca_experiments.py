...
from scripts.evaluate_policy_experiment import evaluate, markdown, write_review_csv
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Single entry point for the remaining RTCA evidence run: Level-0 sanity check, "
            "live-model three-policy Experiment B, automatic conservative evaluation, and "
            "a manual-adjudication sheet."
        )
    )
...
def main() -> int:
    ...
    manifest = {
        ...
        "claim_boundary": (
            "The automatic evaluation is a conservative screen, not final adjudication. "
            "Complete the manual-review CSV before using subtle branch-closure, contamination, "
            "or facilitation claims in the manuscript. No participant or false-memory outcome is measured."
        ),
    }
